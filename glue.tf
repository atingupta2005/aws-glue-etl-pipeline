...
resource "aws_glue_crawler" "aws_glue_custom_csv_crawler" {
  ...
  command = "touch ./tmpfile; aws s3 cp ./tmpfile s3://${var.bucket_for_glue}/tmp/tmpfile; aws s3 cp ./custom_data.csv s3://${var.bucket_for_glue}/sales/custom_data.csv;aws glue start-crawler --name ${self.name}"
...
resource "aws_glue_job" "aws_glue_job_nginx_logs_processing" {
  ...
  command {
    script_location = "s3://${aws_s3_bucket.bucket_for_glue.bucket}/${aws_s3_bucket.bucket_for_glue.bucket.script.key}"
  }
